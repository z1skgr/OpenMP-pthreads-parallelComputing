/*
            ...
            + Name        : newserial.c                          +
            + Author      : Christos Z, 			             +
            + Description : Serial algorithm Smith-Waterman.     +
            ======================================================
 */
#include <stdio.h>
#include <stdlib.h>
...
/*-----------------------------------------------------------*/
    double ExecTime1;       //Variables for performace
    double ExecTime2;
    double ExecTime;
...
/*-----------------------------------------------------------*/
	...
	printf("Hello\n");
	if (argc > 11 || argc < 1){//Wrong arguements
...
/*-----------------------------------------------------------*/
  	int tracestepsNum=0;
  	D=(char*)calloc(Dmax,sizeof(char));
    Q=(char*)calloc(Qmax,sizeof(char));
/*-----------------------------------------------------------*/
    ...
    while(loop>0){
		...
		for(i=0;i<Qc;i++){
			for(j=0;j<Dc;j++){
                calculation(Q,D,scoreTable,i,j,match,mismatch,gap);
