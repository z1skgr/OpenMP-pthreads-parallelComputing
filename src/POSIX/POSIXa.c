...
void* ypologismos_matrix(void *t) {
    int num1;
    int num2;
    int num3;
    int value;
    ...
    for (int i=start_th; i<stop_th; i++) {
        for (int j = 0; j < Dc; j++) {
                if (Q[i] == D[j]) 
...
/*-----------------------------------------------------------*/
    while(loop>0){
        ...
        maxV=0;
    	times=0;
        pthread_t* pthreads = (pthread_t*) malloc (nthreads * sizeof(pthread_t));
        pthread_attr_t attr;
		...
		step_th = Qc/nthreads;
        int count=0;
        for (thesis_str = 0; thesis_str < Qc; thesis_str+=step_th) {
            pthread_create(&pthreads[count], &attr, ypologismos_matrix, NULL);
            
            stop_th = thesis_str+step_th;
            if (stop_th>Qc){
                stop_th = Qc;
            }
            start_th = thesis_str;
    ...
    printf("\n\n\nResults:");
    printf("\nA) Number of pairs Q-D:%d pairs",pairNum);//A
